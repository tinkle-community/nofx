...
func (s *Store) persistSnapshot(traderID string, snapshot Snapshot, flags *featureflag.RuntimeFlags) {
	...
	backend := metrics.BackendMemory
	metrics.IncRiskPersistenceAttemptsWithBackend(traderID, backend)
	start := time.Now()
	if fn, ok := s.persist.Load().(PersistFunc); ok && fn != nil {
		if err := fn(traderID, snapshot); err != nil {
			metrics.IncRiskPersistenceFailuresWithBackend(traderID, backend)
		}
	}
	metrics.ObserveRiskPersistLatencyWithBackend(traderID, time.Since(start), backend)
}
