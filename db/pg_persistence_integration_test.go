...
func withPostgres(t *testing.T, fn func(connStr string)) {
    ...
    defer instance.Terminate(context.Background())

    fn(instance.ConnectionString())
}

const (
    persistWaitTimeout  = 5 * time.Second
...
func waitForPersistedState(t *testing.T, store *RiskStorePG, predicate func(*RiskState) bool) *RiskState {
    ...
    deadline := time.Now().Add(persistWaitTimeout)
    var last *RiskState
    ...
    t.Fatalf("timed out waiting for persisted state, last state: %+v", state)
    return last
}
...
func TestRiskStorePG_ConcurrentWrites(t *testing.T) {
    withPostgres(t, func(connStr string) {
        ...
        loaded := waitForPersistedState(t, store, func(s *RiskState) bool {
            return s.DailyPnL >= float64(workers*iterations-1)
        })
        if loaded == nil {
            t.Fatal("expected loaded state after concurrent writes")
        }

        t.Logf("Final state after %d concurrent writes: DailyPnL=%.2f", workers*iterations, loaded.DailyPnL)
    })
}
