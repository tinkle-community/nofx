...
func printPositionTable(positions []decision.PositionInfo, availableBalance float64) {
	...
	// 计算账号总价值（可用余额 + 所有持仓的保证金 + 未实现盈亏）
	totalEquity := availableBalance
	for _, pos := range positions {
		totalEquity += pos.MarginUsed + pos.UnrealizedPnL
	}

	log.Println("\n📊 持仓详情:")
	log.Printf("💎 账号总价值: %.2f USDT", totalEquity)
	log.Printf("💰 可用余额: %.2f USDT\n", availableBalance)
	log.Println("┌──────────────┬──────┬─────────────────┬──────────────┬──────────┬──────────────┬──────────────┐")
	log.Println("│ 持仓         │ 杠杆 │ 收益            │ 保证金       │ 保证金率 │ 开仓价格     │ 强平价格     │")
	log.Println("├──────────────┼──────┼─────────────────┼──────────────┼──────────┼──────────────┼──────────────┤")
	...
	for _, pos := range positions {
		...
		pnlEmoji := "🤷"
		if pos.UnrealizedPnLPct > 1 {
			pnlEmoji = "📈"
		} else if pos.UnrealizedPnLPct < -1 {
			pnlEmoji = "📉"
		}

		sideText := directionEmoji + pnlEmoji

		// 截断过长的币对名称（保留最多6个字符,超出用..表示）
		displaySymbol := pos.Symbol
		if len(pos.Symbol) > 6 {
			displaySymbol = pos.Symbol[:6] + ".."
		}
		...
		// 截断过长的价格字符串（固定最大长度）
		if len(entryPriceStr) > 12 {
			entryPriceStr = entryPriceStr[:12]
		}
		if len(liqPriceStr) > 12 {
			liqPriceStr = liqPriceStr[:12]
		}

		// 截断过长的收益字符串（固定最大长度）
		if len(profitStr) > 15 {
			profitStr = profitStr[:15]
		}

		// 输出表格行（使用固定宽度格式）
		log.Printf("│ %s %-6s │ %3dx │ %-15s │ %8.2f USDT │ %6.2f%% │ %12s │ %12s │",
			sideText,
			displaySymbol,
			pos.Leverage,
			profitStr,
