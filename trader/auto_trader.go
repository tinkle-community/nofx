...
func NewAutoTrader(config AutoTraderConfig, store *risk.Store, flags *featureflag.RuntimeFlags) (*AutoTrader, error) {
	...
	if maxDailyLoss > 0 && config.InitialBalance > 0 {
		if maxDailyLoss <= 1 {
			maxDailyLoss = config.InitialBalance * maxDailyLoss
		} else {
			maxDailyLoss = config.InitialBalance * maxDailyLoss / 100
		}
	}

	stopMinutes := int(config.StopTradingTime / time.Minute)
	if config.StopTradingTime > 0 && stopMinutes == 0 {
		stopMinutes = 1
	}
	...
	riskEngine := risk.NewEngine(config.ID, riskLimits, flags)
	...
	metrics.ObserveRiskDailyPnL(at.id, 0)
	metrics.SetRiskTradingPaused(at.id, false)
	at.SetTrading(false)

	return at, nil
}

// NewAutoTraderWithPersistence 创建带有状态持久化的自动交易器
func NewAutoTraderWithPersistence(cfg AutoTraderConfig, dbPath string, flags *featureflag.RuntimeFlags) (*AutoTrader, error) {
	...
	runtimeFlags := at.featureFlags
	if runtimeFlags == nil || !runtimeFlags.PersistenceEnabled() {
		return at, nil
	}

	if strings.TrimSpace(dbPath) == "" {
		log.Printf("WARN: [%s] 未提供持久化路径，跳过风险状态持久化", cfg.Name)
		return at, nil
	}

	store, err := db.NewRiskStore(dbPath)
	if err != nil {
		log.Printf("WARN: [%s] 启用风险状态持久化失败: %v", cfg.Name, err)
		return at, nil
	}

	state, err := store.Load()
	if err != nil {
		log.Printf("WARN: [%s] 读取风险状态失败: %v", cfg.Name, err)
		_ = store.Close()
		return at, nil
	}

	at.persistence = store
	log.Printf("💾 [%s] 已启用风险状态持久化 -> %s", cfg.Name, dbPath)

	if state != nil {
		state.TraderID = cfg.ID
		at.restorePersistedState(*state)
		pausedUntil := "-"
		if !state.StopUntil.IsZero() {
			pausedUntil = state.StopUntil.Format(time.RFC3339)
		}
		log.Printf("💾 [%s] 已恢复风险状态，日盈亏 %.2f，暂停至 %s", cfg.Name, state.DailyPnL, pausedUntil)
	}
...
func (at *AutoTrader) Stop() {
	at.SetTrading(false)
	if at.persistence != nil {
		if err := at.persistence.Close(); err != nil {
			log.Printf("WARN: 关闭风险状态持久化失败 [%s]: %v", at.name, err)
		}
		at.persistence = nil
	}
...
func (at *AutoTrader) runCycle() error {
	...
	at.updateBalances(ctx.Account.TotalEquity)
...
func (at *AutoTrader) CanTrade() (bool, string) {
	if at == nil {
		return false, "trader 未初始化"
	}

	now := time.Now()
	stopUntil := at.GetStopUntil()
	if !stopUntil.IsZero() {
		if now.After(stopUntil) {
			...
			if minutes < 1 {
				return false, "风险控制暂停中，剩余不足1分钟"
			}
			return false, fmt.Sprintf("风险控制暂停中，剩余 %.0f 分钟", minutes)
		}
	}

	at.ensureDailyPnLReset(now)

	if at.featureFlags == nil || !at.featureFlags.RiskEnforcementEnabled() || at.riskEngine == nil {
		return true, ""
	}

	state := at.currentRiskState()
	allowed, reason := at.riskEngine.CheckLimits(state)
	if allowed {
		return true, ""
	}

	duration := at.riskEngine.CalculateStopDuration()
	if duration <= 0 {
		duration = time.Duration(at.config.StopTradingTime)
		if duration <= 0 {
	...
	at.persistRiskState("risk_limit_breached")
...
func (at *AutoTrader) ensureDailyPnLReset(now time.Time) {
	lastReset := at.GetLastResetTime()
	if lastReset.IsZero() {
		at.SetLastResetTime(now)
		return
	}
...
func (at *AutoTrader) updateBalances(equity float64) {
	if at == nil {
		return
	}
	...
	if at.mutexProtectionDisabled() {
		at.currentBalance = equity
		if equity > at.peakBalance {
			at.peakBalance = equity
		}
		peak = at.peakBalance
		current = at.currentBalance
	} else {
		at.stateMu.Lock()
		at.currentBalance = equity
		if equity > at.peakBalance {
	...
	metrics.ObserveRiskDrawdown(at.id, drawdown)
	at.persistRiskState("equity_update")
}

func (at *AutoTrader) currentRiskState() risk.State {
	if at == nil {
		return risk.State{}
	}
	...
	if at.mutexProtectionDisabled() {
		return risk.State{
			...
			PeakBalance:    at.peakBalance,
			CurrentBalance: at.currentBalance,
...
func (at *AutoTrader) snapshotRiskState() db.RiskState {
	snapshot := db.RiskState{TraderID: at.id}
	if at.mutexProtectionDisabled() {
		snapshot.DailyPnL = at.dailyPnL
		snapshot.PeakBalance = at.peakBalance
		snapshot.CurrentBalance = at.currentBalance
		snapshot.LastResetTime = at.lastResetTime
		snapshot.StopUntil = at.stopUntil
	} else {
		at.stateMu.RLock()
		snapshot.DailyPnL = at.dailyPnL
		snapshot.PeakBalance = at.peakBalance
...
func (at *AutoTrader) persistRiskState(reason string) {
	if at == nil || at.persistence == nil {
		return
	}
	if at.featureFlags != nil && !at.featureFlags.PersistenceEnabled() {
		return
	}

	snapshot := at.snapshotRiskState()
	traceID := fmt.Sprintf("%s-%d", at.id, time.Now().UnixNano())
	if err := at.persistence.Save(&snapshot, traceID, reason); err != nil {
		log.Printf("WARN: 风险状态持久化失败 [%s]: %v", at.name, err)
	}
}
...
func (at *AutoTrader) restorePersistedState(state db.RiskState) {
	...
	if at.mutexProtectionDisabled() {
		...
		if state.PeakBalance > 0 {
			at.peakBalance = state.PeakBalance
		}
		if state.CurrentBalance > 0 {
			at.currentBalance = state.CurrentBalance
		}
	...
	metrics.ObserveRiskDailyPnL(at.id, state.DailyPnL)
	paused := !state.StopUntil.IsZero() && state.StopUntil.After(time.Now())
	metrics.SetRiskTradingPaused(at.id, paused)
}
...
// UpdateDailyPnL applies a delta to the tracked daily PnL and records metrics
// alongside optional persistence when enabled.
func (at *AutoTrader) UpdateDailyPnL(delta float64) float64 {
	...
	metrics.ObserveRiskDailyPnL(at.id, value)
	at.persistRiskState("daily_pnl_update")
	return value
}
...
func (at *AutoTrader) ResetDailyPnL(atTime time.Time) {
	...
	metrics.ObserveRiskDailyPnL(at.id, 0)
	at.persistRiskState("daily_reset")
}
...
func (at *AutoTrader) SetStopUntil(until time.Time) {
	...
	paused := !until.IsZero() && until.After(time.Now())
	metrics.SetRiskTradingPaused(at.id, paused)
	at.persistRiskState("stop_until_update")
}
...
func (at *AutoTrader) SetLastResetTime(ts time.Time) {
	...
	at.persistRiskState("last_reset_update")
}
