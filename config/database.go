...
func (d *Database) createTables() error {
	...
	alterQueries := []string{
		...
		`ALTER TABLE ai_models ADD COLUMN is_deleted BOOLEAN DEFAULT 0`,
		`ALTER TABLE exchanges ADD COLUMN is_deleted BOOLEAN DEFAULT 0`,
	}
...
func (d *Database) migrateExchangesTable() error {
	...
	_, err = d.db.Exec(`
		INSERT INTO exchanges_new (
			id, user_id, name, type, enabled, api_key, secret_key, testnet, is_deleted,
...
func (d *Database) UpdateAIModel(userID, id string, enabled bool, apiKey, customAPIURL, customModelName string, deleteFlag bool) error {
	...
	if err == nil {
		if deleteFlag {
			names, err := d.tradersUsingAIModel(userID, existingID, provider)
			if err != nil {
				return err
			}
			if len(names) > 0 {
				return fmt.Errorf("AI模型正在被交易员使用，需先删除交易员：{%s}", strings.Join(names, "、"))
			}
		}
		...
		if deleteFlag {
			_, err = d.db.Exec(`
				UPDATE ai_models 
				SET api_key = '', custom_api_url = '', custom_model_name = '', 
				    is_deleted = 1, updated_at = datetime('now')
				WHERE id = ? AND user_id = ?
			`, existingID, userID)
			return err
		}

		// 找到了现有配置，更新它并确保标记为未删除
		_, err = d.db.Exec(`
			UPDATE ai_models 
			SET enabled = ?, api_key = ?, custom_api_url = ?, custom_model_name = ?, 
...
func (d *Database) UpdateExchange(userID, id string, enabled bool, apiKey, secretKey string, testnet bool, hyperliquidWalletAddr, asterUser, asterSigner, asterPrivateKey string, deleteFlag bool) error {
	log.Printf("🔧 UpdateExchange: userID=%s, id=%s, enabled=%v", userID, id, enabled)

	if deleteFlag {
		names, err := d.tradersUsingExchange(userID, id)
		if err != nil {
			return err
		}
		if len(names) > 0 {
			return fmt.Errorf("交易所正在被交易员使用，需先删除交易员：{%s}", strings.Join(names, "、"))
		}
		...
		`, id, userID)
		if err != nil {
			log.Printf("❌ UpdateExchange: 删除标记失败: %v", err)
			return err
		}

		rowsAffected, err := result.RowsAffected()
		if err != nil {
			log.Printf("❌ UpdateExchange: 获取删除影响行数失败: %v", err)
			return err
		}

		if rowsAffected == 0 {
			log.Printf("ℹ️ UpdateExchange: 删除请求未找到记录 id=%s user=%s", id, userID)
		} else {
			log.Printf("🗑️ UpdateExchange: 已标记交易所 %s 为删除状态", id)
		}
		return nil
	}
	...
	if rowsAffected == 0 {
		...
		_, err = d.db.Exec(`
			INSERT INTO exchanges (id, user_id, name, type, enabled, api_key, secret_key, testnet, 
			                       hyperliquid_wallet_addr, aster_user, aster_signer, aster_private_key, created_at, updated_at, is_deleted)
...
func (d *Database) tradersUsingAIModel(userID, modelID, provider string) ([]string, error) {
	var rows *sql.Rows
	var err error

	switch {
	case modelID != "" && provider != "" && modelID != provider:
		rows, err = d.db.Query(`
			SELECT name FROM traders 
			WHERE user_id = ? AND (ai_model_id = ? OR ai_model_id = ?)
		`, userID, modelID, provider)
	case modelID != "":
		rows, err = d.db.Query(`
			SELECT name FROM traders 
			WHERE user_id = ? AND ai_model_id = ?
		`, userID, modelID)
	case provider != "":
		rows, err = d.db.Query(`
			SELECT name FROM traders 
...
func (d *Database) tradersUsingExchange(userID, exchangeID string) ([]string, error) {
	rows, err := d.db.Query(`
		SELECT name FROM traders 
...
func (d *Database) GetTraderConfig(userID, traderID string) (*TraderRecord, *AIModelConfig, *ExchangeConfig, error) {
	...
	err := d.db.QueryRow(`
		...
		FROM traders t
		JOIN ai_models a ON t.ai_model_id = a.id AND t.user_id = a.user_id AND COALESCE(a.is_deleted, 0) = 0
		JOIN exchanges e ON t.exchange_id = e.id AND t.user_id = e.user_id AND COALESCE(e.is_deleted, 0) = 0
